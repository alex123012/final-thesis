...
def df_for_barplot(df: pd.DataFrame) -> pd.DataFrame:
    return pd.crosstab(df.REF, df.ALT).melt(ignore_index=False, value_name=Y_COL).reset_index()

def plot_ref_alt_barplot(df: pd.DataFrame, result_barplot: str, name: str, x: str = X_COL, y: str = Y_COL, hue: str = None):
    ...
    sns.barplot(
        ...
        ax=ax,
        order=data.groupby([X_COL, Y_COL]).apply(lambda df: df[Y_COL].sum()).reset_index().groupby(X_COL).apply(lambda df: df[0].sum()).sort_values().index
    )
    mx, mn = data[Y_COL].max(), data[Y_COL].min()-5
    ax.set_yticks([i for i in range((mn > 0 and mn-5) or 0, (mx > 5 and mx+5) or mx+1, (mx > 5 and 5) or 1)])
...
def main():
    ...
    plot_ref_alt_barplot(df=df, name=name, result_barplot=result_barplot, x=X_COL, y=Y_COL)
